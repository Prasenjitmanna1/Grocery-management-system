...
//to check if already exist
    $check_item=  array_column( $_SESSION['pcart'],'iname');
    ...
    // $check_item=  array_column( $_SESSION['pcart'],'iname');
    // $check_stock  array_column( $_SESSION['pcart'],'iqty');
    // if(in_array($name,$check_item)){
    //     echo"
    //     <script>
    //     alert('Item already exist ');
...
if(isset($_POST["remove"])){
    foreach($_SESSION['pcart'] as $key => $value){
       if($value['iname']=== $_POST['item']){
           ...
           // rearrange key value
           $_SESSION['pcart'] = array_values($_SESSION['pcart'] );
